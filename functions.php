...
if ( ! function_exists( 'fromscratch_setup' ) ) :
	 ...
	 */
	function fromscratch_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on fromscratch, use a find and replace
		 * to change 'fromscratch' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'fromscratch' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'fromscratch_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'fromscratch_setup' );
 ...
 */
function fromscratch_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'fromscratch_content_width', 640 );
}
add_action( 'after_setup_theme', 'fromscratch_content_width', 0 );
...
function fromscratch_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'fromscratch' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'fromscratch' ),
...
add_action( 'wp_enqueue_scripts', 'fromscratch_scripts' );
